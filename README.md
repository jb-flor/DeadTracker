...
- **`static/index.html`** — a single-page frontend that resolves whatever you type into the input, then polls the endpoints above and displays:
  ...
  - A per-match view — a team-vs-enemy net worth chart, a net-worth/K-D-A curve, and item purchases/sales plus ability level-ups plotted against match time — the chart is the only way to inspect the build now , so every point has a styled hover tooltip with the item's in-game shop icon, name, and timing. Shows for any match you click, and for both players at once in Compare Build
